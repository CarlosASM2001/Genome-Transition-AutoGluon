...
def process_gene(gene: GeneRecord, output: CsvOutput, flank_size: int, negatives_per_positive: int, rng: random.Random) -> None:
    ...
    for transcript_exons in gene.transcripts:
        ...
        if ze_sequence and write_csv_row(output["ze"], gene, first_exon_start, ze_sequence, True):
        ...
        if ez_sequence and write_csv_row(output["ez"], gene, last_exon_end, ez_sequence, True):
        ...
        for exon_left, exon_right in zip(ordered_exons, ordered_exons[1:]):
            ...
            if ei_sequence and write_csv_row(output["ei"], gene, intron_start, ei_sequence, True):
            ...
            if ie_sequence and write_csv_row(output["ie"], gene, exon_start, ie_sequence, True):
