...
│   ├── extract_data.py            # Script that reads Ensembl files and writes CSVs
...
│
├── bio-env.txt                    # Requirements enviroment
├── LICENSE                        # MIT License
└── README.MD                      # This file
```
...
Raw Ensembl gene files are parsed to identify positive transition positions from exon coordinates. For each positive, a negative sample is generated at a random nearby position (controlled distance) to create balanced binary datasets. You need to run `principal_extraction_data.ipynb` you can change some parameters

```bash
    input_dir=Path("../data_ensembl"),
    output_dir=Path("../data"),
    input_file=[],          # or ["3-187668812-187670494.txt"]
    flank_size=1000,
...
Generate the confusion matrix plot and summary CSV by running `06_confusion_matrices.ipynb`
...
|--------|------|-------------|
| `GET` | `/` | API info and configured transitions |
